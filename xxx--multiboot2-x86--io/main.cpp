...
namespace
{
  ...
  x86::port<std::uint8_t> idle { 0x80 };
}
...
namespace
{
  struct pic
  {
    x86::port<std::uint8_t> command;
    x86::port<std::uint8_t> data;
  };

  pic master_pic { 0x20, 0x21 };
  pic slave_pic  { 0xA0, 0xA1 };
}
...
void __attribute__((fastcall)) main ( std::uint32_t magic, multiboot2::information_list & mbi )
{
  ...
  master_pic.data.write(pc::icw1(true, false, false));
  slave_pic.data.write(pc::icw1(true, false, false));
  idle.write(0);
  master_pic.data.write(pc::icw2(0x20));
  slave_pic.data.write(pc::icw2(0x28));
  idle.write(0);
  master_pic.data.write(pc::icw3(0b00000100));
  slave_pic.data.write(pc::icw3(2));
  idle.write(0);
  master_pic.data.write(pc::icw4(true, false, false, false, false));
  slave_pic.data.write(pc::icw4(true, false, false, false, false));
  idle.write(0);
